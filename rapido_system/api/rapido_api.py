...
def generate_livekit_token(room_name: str, participant_name: str) -> str:
    ...
    # LiveKit credentials from config / env
    cfg = Config()
    LIVEKIT_API_KEY = getattr(cfg, 'LIVEKIT_API_KEY', '')
    LIVEKIT_API_SECRET = getattr(cfg, 'LIVEKIT_API_SECRET', '')
...
async def generate_livekit_token_endpoint(request: LiveKitTokenRequest):
    ...
    try:
        ...
        cfg = Config()
        livekit_url = getattr(cfg, 'LIVEKIT_URL', 'wss://agent-dev-tzdsmuu4.livekit.cloud')
        try:
            masked_key = (getattr(cfg, 'LIVEKIT_API_KEY', '')[:4] + "***" + getattr(cfg, 'LIVEKIT_API_KEY', '')[-3:]) if getattr(cfg, 'LIVEKIT_API_KEY', '') else ""
            logger.info(f"🔗 Token service LiveKit URL: {livekit_url} | API key: {masked_key}")
        except Exception:
            pass
