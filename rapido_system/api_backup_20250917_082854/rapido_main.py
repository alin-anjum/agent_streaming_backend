...
class RapidoMainSystem:
    ...
    def __init__(self, config_override: dict = None):
        ...
        # Post-slide padding (silence) state
        self._post_slide_padding_frames_remaining = 0
        self._padding_next_action = None  # {'type': 'embed', 'id': ...} or {'type': 'advance'}
        self._last_published_cv_frame = None

    def _now_ms(self) -> int:
        return int(time.time() * 1000)

    async def _publish_control_event(self, payload: dict):
        """Send a control message over LiveKit data channel (topic 'avatar_events')."""
        try:
    ...
    async def send_stream_starting(self):
        await self._publish_control_event({
            'id': str(uuid.uuid4()),
    ...
    async def _connect_livekit_optimized(self):
        ...
        try:
            ...
            # LiveKit credentials from config
            LIVEKIT_URL = getattr(self.config, 'LIVEKIT_URL', '')
            LIVEKIT_API_KEY = getattr(self.config, 'LIVEKIT_API_KEY', '')
            LIVEKIT_API_SECRET = getattr(self.config, 'LIVEKIT_API_SECRET', '')
            try:
                masked_key = (LIVEKIT_API_KEY[:4] + "***" + LIVEKIT_API_KEY[-3:]) if LIVEKIT_API_KEY else ""
                logger.info(f"🔗 LiveKit URL: {LIVEKIT_URL} | API key: {masked_key}")
            except Exception:
                pass
    ...
    async def continuous_frame_delivery(self, frame_processor):
        ...
        try:
            ...
            # Send stream starting event to frontend before first frame publish
            try:
                await self.send_stream_starting()
                logger.info("📡 Stream starting event sent to frontend")
            except Exception as e:
                logger.warning(f"Failed to send stream starting event: {e}")
                ...
                try:
                    ...
                    if frame_data["type"] == "video":
                                    ...
                                    try:
                                        if hasattr(self, '_current_slide_expected') and isinstance(self._current_slide_expected, int) and self._current_slide_expected > 0:
                                            if self._current_slide_frame_loops >= self._current_slide_expected:
                                                # Start post-slide padding: 0.75s at 25fps = ~19 frames
                                                self._post_slide_padding_frames_remaining = 19
                                                # Decide next action after padding finishes
                                                try:
                                                    embed_id = self._next_embed_after_current()
                                                except Exception:
                                                    embed_id = None
                                                if embed_id:
                                                    self._padding_next_action = {'type': 'embed', 'id': embed_id}
                                                else:
                                                    self._padding_next_action = {'type': 'advance'}
                                                # Prevent repeated triggers for same slide
                            ...
                            if hasattr(self, 'last_slide_frame') and self.last_slide_frame is not None:
                                ...
                                if hasattr(self, '_current_slide_frame_loops'):
                                    ...
                                    try:
                                        if hasattr(self, '_current_slide_expected') and isinstance(self._current_slide_expected, int) and self._current_slide_expected > 0:
                                            if self._current_slide_frame_loops >= self._current_slide_expected:
                                                # Start post-slide padding in duplicate path too
                                                self._post_slide_padding_frames_remaining = 19
                                                try:
                                                    embed_id = self._next_embed_after_current()
                                                except Exception:
                        ...
                        if slide_frame is not None:
                            ...
                            # When in post-slide padding period, send last published frame with silent audio
                            if getattr(self, '_post_slide_padding_frames_remaining', 0) > 0 and not getattr(self, 'paused_for_embed', False):
                                try:
                                    # Use last published frame if available, else use current composed
                                    pad_frame = self._last_published_cv_frame if self._last_published_cv_frame is not None else cv_frame
                                    silent_samples = int(0.04 * 16000)  # ~40ms chunk in samples
                                    silent_audio = (np.zeros(silent_samples, dtype=np.int16)).tobytes()
                                    await self.publish_frame_to_livekit(pad_frame, silent_audio)
                                except Exception as _:
                                    pass
                                finally:
                                    self._post_slide_padding_frames_remaining -= 1
                                    # When padding completes, perform next action
                                    if self._post_slide_padding_frames_remaining <= 0:
                                        action = self._padding_next_action or {}
                                        self._padding_next_action = None
                                        if action.get('type') == 'embed':
                                            await self._pause_for_embed(action.get('id'))
                                            self._deferred_advance = True
                                        elif action.get('type') == 'advance' and self.slide_control_queue is not None:
                                            self._advance_signal_time = time.time()
                                            try:
                                                self.slide_control_queue.put_nowait("advance")
                                            except Exception:
                                                pass
                            # Skip publishing during embed pause; do not continuously drain queues here
                            elif getattr(self, 'paused_for_embed', False):
                                self.last_frame_data = None
                                # No-op: one-time drain is done at pause; let queues accumulate for catch-up decisions
                            else:
                                await self.publish_frame_to_livekit(cv_frame, pcm_audio)
                                # Save last published frame for padding reuse
                                self._last_published_cv_frame = cv_frame
